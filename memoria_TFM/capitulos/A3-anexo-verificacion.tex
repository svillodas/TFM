...
La tabla~\ref{table:validacion-nodo} recoge la repetibilidad de las características sobre el
...
La tabla~\ref{table:resiliencia} recoge el comportamiento de los mecanismos de integridad
durante la campaña EXP-002. Los contadores de salud descritos en el anexo~\ref{anexo:software} permiten cuantificarlo sin instrumentación añadida.
...
anexo~\ref{anexo:software}, según el cual descartar la ráfaga completa ante cualquier fallo
...
La tabla~\ref{table:protocolo} recoge el resultado.
...
descartaron por ese motivo \textbf{sin haberlo medido}. La tabla~\ref{table:coste-nodo} recoge
la medición.
...
establece su rendimiento. La tabla~\ref{table:cobertura-modos} recoge el resultado.
...
los datos disponibles no permiten resolver, y que la tabla~\ref{table:compromiso} resume.
...
permite compilarlas en un ordenador y contrastarlas frente a la implementación de referencia
...
\mbox{2,1$\cdot$10$^{-7}$} en LOF), tal como resume la tabla~\ref{table:verificacion-nodo}.
...
La tabla~\ref{table:coste-inferencia} recoge el coste medido de cada alternativa.
...
firmware ni repetir ninguna campaña. La tabla~\ref{table:politica} recoge esa evaluación.
