...
La tabla~\ref{table:sensores} recoge los sensores seleccionados y la magnitud que aporta
...
tabla~\ref{table:requisitos} expresa esa elección en términos de requisitos medibles derivados
...
según recoge la tabla~\ref{table:muestreo}.
...
de los tres coeficientes centrales. La tabla~\ref{table:estimador} recoge la mejora
...
de modo que pueden verificarse numéricamente en un ordenador frente a señales de
...
La tabla~\ref{table:variables} recoge las nueve variables del canal lento con su unidad y
...
La tabla~\ref{table:caracteristicas} recoge las características que el nodo extrae de cada
...
el repositorio del trabajo~\cite{repotfm}, dentro de la carpeta de análisis, cuyo fichero de
documentación recoge la justificación de cada decisión de limpieza y de derivación de
características.
...
observaciones del estado nominal. La tabla~\ref{table:modelos} resume su fundamento.
...
La tabla~\ref{table:resonancia-efecto} resume el efecto medido de ambas medidas sobre la misma señal.
